...
NEW_LOG_DIR="$LOG_DIR/GPU_Util/$TYPE/$FILENAME/$DATAFOLDER/$AUG/$MODEL/epoch${EPOCH}/b${BATCH}/worker${WORKERS}/thread${THREADS}"
...
python_pid=$!
python $SCRIPT_DIR/gpu_util_logger.py ${NEW_LOG_DIR} &
wait $python_pid
