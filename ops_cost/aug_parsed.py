...
#     if re.search(io_time_pattern, line) is not None:  # start log
#         replace_txt = line.replace('\t', ' ')
#         test = replace_txt.split(' ')
#         info = list(filter(lambda x: x != "", test))
                
#         io_time = find_value(info, "I/O_time:")
#         image_loglet["I/O time"] = io_time
...
dir_names = glob.glob('./aug_parsed/*.csv')
...
avg_df.dropna().to_csv(f"./total_summary.csv",
                   sep=',', na_rep='NA')
