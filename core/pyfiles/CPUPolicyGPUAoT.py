...
import customUtils
import numpy as np
import localManagerGPUAoT as PinData
...
def main():
    ...
    if args.arch.startswith('vit'):
        file_name = f"GPUAoT_worker_{args.workers}_Mem_Trace_vit_{args.augs}_b{args.batch_size}.csv"
    else:
        file_name = f"GPUAoT_worker_{args.workers}_Mem_Trace_{args.arch}_{args.augs}_b{args.batch_size}.csv"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    CPU_IDS_PER_GPU = CPU_NUM // NUM_GPU # CPU_NUM // NUM_GPU (6)
    ...
    if args.tracefetch:
        ...
        #     # workers=['0.0.0.0:51234'], # For Cerrotoree
    ...
    if args.seed is None:
        ...
        train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=CPU_NUM, aggr_shape_hint=template_shape, worker_batch_size=args.threads, aug_type=args.augs)
        ...
        train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=CPU_NUM, aggr_shape_hint=template_shape, worker_init_fn=seed_worker, generator=g, worker_batch_size=args.threads, prefetch_factor=1)
        # dummy_loader = BaseData.DataLoader(
        #         train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank,
