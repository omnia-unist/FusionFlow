...
import customUtils
import numpy as np
import ReactiveRemote as PinData
...
def main():
    ...
    logger_fname_memory = parent_dir + '/_log_GPU_Mem.csv'
    logger_fname_memory = parent_dir + f'/ReacitveRemote_worker_{args.workers}_Mem_Trace_{args.arch}_b{args.batch_size}_{args.augs}.csv'
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    CPU_IDS_PER_GPU = CPU_NUM // 6 # CPU_NUM // NUM_GPU (6)
    ...
    if args.tracefetch:
        ...
        print("SegregatedAugImageFolder is chosen!!", flush=True)
        # train_dataset = datasets.ImageFolder(
        #     traindir,
        #     augmentations_composed
        # )
        train_dataset = customUtils.SegregatedAugImageFolder(
            traindir,
            augmentations_composed
        )
        # Here We Turn on/off remote dataloading!
        remote_dataset = None    
        # Multiconnection style
        ...
        remote_dataset = PinData.dataset.RPCDataset(
            # workers=remote_worker_address,
            # workers=['0.0.0.0:45123'], # For Cerrotoree
            workers=['10.20.17.118:45123'], # For Lohtse
            dataset=None,
            # dataset=datasets.ImageFolder,
            # num_workers=len(remote_worker_address),
            num_workers=4,
    ...
    if args.seed is None:
        ...
        train_loader = PinData.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank, remote_dataset=remote_dataset,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=CPU_NUM, aggr_shape_hint=template_shape, worker_batch_size=args.threads, aug_type=args.augs, control_queue=control_queue)
        # dummy_loader = BaseData.DataLoader(
        ...
        train_loader = PinData.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank, aug_type=args.augs, remote_dataset=remote_dataset,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=CPU_NUM, aggr_shape_hint=template_shape, worker_init_fn=seed_worker, generator=g, worker_batch_size=args.threads, prefetch_factor=1, control_queue=control_queue)
        # dummy_loader = BaseData.DataLoader(
        #         train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        # train(train_loader, dummy_loader, model, criterion, optimizer, epoch, args)
        train(train_loader, model, criterion, optimizer, epoch, args, control_queue)
...
def train(train_loader, model, criterion, optimizer, epoch, args, control_queue):
    ...
    # variable for early printing
    prev_batchnum = 0
    i = 0
    dummy_iter = -5
    gpu_id = args.gpu

    if epoch == 0:  # Dummy Loader
        i = dummy_iter
        gpu_device = torch.device('cuda', index=gpu_id)
        if args.arch.startswith('vit'):
            test_data = \
                torch.zeros(args.batch_size, 3, 224, 224, dtype=torch.float32, device=gpu_device), \
                torch.zeros(args.batch_size, dtype=torch.int64, device=gpu_device)
    ...
    print(f"args.prof {args.prof}", flush=True)
    # for i, (images, target) in enumerate(train_loader):
    # train_loader_len = int(math.ceil(train_loader._size / args.batch_size))
    GC_Helper = train_loader.manager_call()
    
    GC_Helper.worker_control(8, -1, 0)  # Set 4 workers

    train_loader_len = len(train_loader)
    while i < train_loader_len:
        if args.prof >= 0 and i == args.prof:
            print("Profiling begun at iteration {}".format(i))
            torch.cuda.cudart().cudaProfilerStart()
        if PinData.USE_SHARED_MEMORY:
            torch.cuda.synchronize()
        if args.prof >= 0: torch.cuda.nvtx.range_push("Data loading")
        if i == 0 and PinData.USE_SHARED_MEMORY:
            # pass
            # torch._C._cuda_renew_shared_memory()
            torch.cuda.renew_shared_memory()
        if PinData.USE_SHARED_MEMORY:
            torch.cuda.synchronize()

        if i < 0:  # Dummy PyTorch Run
            print(f"{os.getpid()} PyTorch Dummy!!!!!!!!!!!!!!!!!-------------------------------------", flush=True)
            images, target = test_data
        elif i >= 0:  # DALI
            print(f"{os.getpid()} DALI WHOLE START!!!!!!!!!!!!!!!-------------------------------------", flush=True)
            if PinData.USE_SHARED_MEMORY:
                torch.cuda.mark_shared_memory()
            images, target = next(train_iterator, (None, None))
            if PinData.USE_SHARED_MEMORY:
                torch.cuda.unmark_shared_memory()
            if images is None:
                break
            # time.sleep(10)
            print(f"{os.getpid()} DALI WHOLE END!!!!!!!!!!!!!!!!!-------------------------------------", flush=True)
        print(f"----------PyTorch|SEPARATOR-------- {i}", flush=True)
        break_point = 510
        if i == break_point: 
            break

        if i == 100:  # Add 4 local workers
            print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
            GC_Helper.worker_control(4, 1, 1)
        elif i == 200:  # Add remote workres
            print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
            control_queue.put(True)
        elif i == 300:  # Remote remote workers
            print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
            control_queue.put(False)
        elif i == 400:  # Remote 4 local workers
            print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
            GC_Helper.worker_control(4, -1, 4)

        # if i == 100:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(2, -1, 1)
        # elif i == 150:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(2, 1, 2)
        # elif i == 100:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(4, 1, 3)
        # elif i == 200:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(4, 1, 5)
        # elif i == 300:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(4, -1, 6)            
        # elif i == 400:
        #     print(f"GPU {args.gpu} Iteration {i} start : {time.time()}")
        #     GC_Helper.worker_control(4, -1, 4)
        #     # gc.deleting(Global_Manager, args.gpu, 3)
        # else:
        #     GC_Helper.worker_control(0, 0, -1)
        ...
        if args.prof >= 0: torch.cuda.nvtx.range_push("Body of training {}".format(i))        
        ...
        with torch.cuda.amp.autocast():
            ...
            optimizer.step()
            if args.prof >= 0: torch.cuda.nvtx.range_pop()            # torch.cuda.renew_shared_memory()
        ...
        # if i % 100 == 0:
        #     print(f"Current iteration is {i}. Provide intermediate throughput")
        #     for j in range(prev_batchnum, i):
        #         data_arr[j][0].synchronize()
        #         datatime = data_arr[j][0].elapsed_time(data_arr[j][1])/1000
        #         batch_arr[j][1].synchronize()
        #         itertime = batch_arr[j][0].elapsed_time(batch_arr[j][1])/1000
        #         batch_time.update(itertime)

        #         throughput.update(args.batch_size/itertime)
        #         progress.display(j, dtimes[j])
        #     prev_batchnum = i
