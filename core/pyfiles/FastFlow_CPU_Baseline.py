...
import numpy as np
# import PinData
import RPCDataLoader as PinData
from RPCDataLoader import REMOTE_DECODE_PREP_COLLATE_PIPE, REMOTE_DECODE_PREP_PIPE, REMOTE_PREP_PIPE, REMOTE_READ_PREP_WITH_DECODE_PIPE, REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE

name_of_pipe = {REMOTE_DECODE_PREP_COLLATE_PIPE: "REMOTE_DECODE_PREP_COLLATE_PIPE", 
                REMOTE_PREP_PIPE: "REMOTE_PREP_PIPE", 
                REMOTE_DECODE_PREP_PIPE: "REMOTE_DECODE_PREP_PIPE",
                REMOTE_READ_PREP_WITH_DECODE_PIPE: "REMOTE_READ_PREP_WITH_DECODE_PIPE",
                REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE: "REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE"}
...
def main():
    ...
    parent_dir = "/home/mansur/Manticore/manticore/core/Utilization_log"
    
    if args.arch.startswith('vit'):
        file_name = f"Baseline_worker_{args.workers}_Mem_Trace_vit_b{args.batch_size}.csv"
    else:
        file_name = f"Baseline_worker_{args.workers}_Mem_Trace_{args.arch}_b{args.batch_size}.csv"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif args.arch == "Transformer_ASR":
        model = FastFlowModels.Transformer(num_hid=200,
                                           num_head=2,
    ...
    if args.arch == "Transformer_ASR":
        ...
        if args.tracefetch:
            ...
            # Here We Turn on/off remote dataloading!
            # remote_dataset = None
            remote_worker_address = []
            num_remote_workers = 4
            for i in range(num_remote_workers):
                remote_worker_address.append(f"0.0.0.0:{45123 + i}")  # For CerroTorre
                # remote_worker_address.append(f"10.20.17.118:{45123 + i}")  # For Lhotse

            remote_dataset = PinData.dataset.RPCDataset(
                workers=remote_worker_address, # For Cerrotoree
                # workers=['10.20.17.118:45123'], # For Lohtse
                dataset=datasets.ImageFolder, # This is used if Remote Machine has a copy of DB, or inter batching is used
                # dataset=None,  # This is used if Remote Machine does not have a copy of DB
                # pipeline_mode=REMOTE_PREP_PIPE,
                # pipeline_mode=REMOTE_DECODE_PREP_PIPE,
                # pipeline_mode=REMOTE_DECODE_PREP_COLLATE_PIPE,
                
                # pipeline_mode=REMOTE_DECODE_PREP_PIPE, 
                # pipeline_mode=REMOTE_READ_PREP_WITH_DECODE_PIPE,
                pipeline_mode=REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE,
                # offload_coefficient=0.2,
                offload_coefficient=1,
                # offload_coefficient=0.8064721948248046,
                
                transform=augmentations_composed,
                root=traindir,
                loader=customUtils.imagenet_21k_loader
            )
    ...
    if args.seed is None:
        ...
        if args.arch == "Transformer_ASR":
            ...
            train_loader = PinData.DataLoader(
                train_dataset, remote_dataset=remote_dataset, prefetch_factor=prefetch_factor, batch_size=args.batch_size, shuffle=(train_sampler is None), rank=cur_rank, num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, aggr_shape_hint=template_shape)
        # # train_loader = torch.utils.data.DataLoader(
        ...
        if args.arch == "Transformer_ASR":
            train_loader = torch.utils.data.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, worker_init_fn=seed_worker, generator=g, aggr_shape_hint=template_shape)
        else:
            train_loader = PinData.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank=cur_rank, prefetch_factor=prefetch_factor,
                num_workers=args.workers, remote_dataset=remote_dataset, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, worker_init_fn=seed_worker, generator=g, aggr_shape_hint=template_shape)
        # train_loader = torch.utils.data.DataLoader(
        #     train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Profiling for FastFlow
        if epoch == args.start_epoch and remote_dataset is not None:
            profile_fastflow(train_loader, model, criterion, optimizer, epoch, args)
...
def profile_train(train_loader, model, criterion, optimizer, epoch, args, break_point=200):
    '''Returns Cycles, TrainTime, Time'''
    batch_time = AverageMeter('Time', ':6.6f')
    data_time = AverageMeter('Data', ':6.6f')
    throughput = AverageMeter('Throughput', ':6.3f')
    wait_time = AverageMeter('WaitTime', ':6.6f')
    ...
    profiled_time = 0  # Model + Data Processing Throughput
    profiled_traintime = 0 # Only Model Thoughput
    for i in range(break_point):
        # Heelim
        # torch.cuda.synchornize : waits until the completion of all work currently caputred in this event
        # This prevents the CPU thread from proceeding until the event completes.
        ...
        train_time.update(traintime)
        if i >= dummy_runs:
            print(itertime, traintime, flush=True)
            profiled_time += args.batch_size/itertime
            profiled_traintime += args.batch_size/traintime
        throughput.update(args.batch_size/itertime)
        # progress.display(i, dtimes[i])
    break_point -= dummy_runs
    profiled_time /= break_point
    profiled_traintime /= break_point
    print("Performance", "TrainOnly:", profiled_traintime, "Train+Data:", profiled_time, flush=True)
    if __debug__:
        log.debug(f"GPU {args.gpu} Arrnage time for iterations done")
    return cpu_end.user - cpu_start.user, profiled_traintime, profiled_time


def profile_fastflow(train_loader, model, criterion, optimizer, epoch, args, break_point=50):
    print("FastFlow Profiling", flush=True)
    # pipelines = [REMOTE_PREP_PIPE, REMOTE_DECODE_PREP_PIPE, REMOTE_DECODE_PREP_COLLATE_PIPE]. If there is no DB there
    # pipelines = [REMOTE_PREP_PIPE, REMOTE_READ_PREP_WITH_DECODE_PIPE, REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE] # If there is DB there
    pipelines = [REMOTE_READ_PREP_WITH_DECODE_COLLATE_PIPE, REMOTE_READ_PREP_WITH_DECODE_PIPE] # If there is DB there

    
    # Measure Throughput of Only Model (TrainTime only)
    # Measure Throughput of Model + Data Preparation (Time)
    # Measure CPU cycles of Model without Offloading
    train_loader._apply_remote_configuration(pipeline_mode=None, offload_ratio=0)
    Pcycle, Gthp, Lthp = profile_train(train_loader, model, criterion, optimizer, epoch, args, break_point=break_point)

    # Saved Format (Ocycle, Rthp)
    profiled_stats = []
    # Values that are calculated using FastFlow Algorithm 1
    calculated_values = []
    for i, pipe in enumerate(pipelines):
        print("Profiling Pipe", name_of_pipe[pipe], flush=True)
        train_loader._apply_remote_configuration(pipeline_mode=pipe, offload_ratio=1)

        # Cpu Cycles of training with offloading
        Ocycle, _, Rthp = profile_train(train_loader, model, criterion, optimizer, epoch, args, break_point=break_point)
        print("Pipe", name_of_pipe[pipe], "Ocycle", Ocycle, "Throughput:", Rthp, flush=True)
        # Measure Remote Thoughput of Model
        profiled_stats.append((Ocycle, Rthp))
        # Measure the value
        value = Lthp * (1 - Ocycle / Pcycle) + Rthp
        calculated_values.append(value)

    candidate_pipe = calculated_values.index(max(calculated_values)) 
    Ocycle, Rthp = profiled_stats[candidate_pipe]
    Upper = Rthp
    Lower = (Gthp - Lthp) * (1 + (Ocycle / Pcycle))
    if Lower < Upper:
        OffRatio = Upper / Gthp
    else:
        OffRatio = Rthp / (Lthp + Rthp)
    print("Pipeline", name_of_pipe[pipelines[candidate_pipe]], ", OffRatio", OffRatio, flush=True)
    train_loader._apply_remote_configuration(pipeline_mode=pipelines[candidate_pipe], offload_ratio=OffRatio)
    return
