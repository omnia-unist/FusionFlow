...
# from localManagerCPUGPUDirectAggrOffloadTaskExecution import rpc_async
from ReactiveRemote import rpc_async
...
class RPCDataset(Dataset):
    ...
    def __init__(
        self, workers: List[str], dataset: Dataset[T], num_workers=1, *args, **kwargs
    ):
    ...
    def initiate(self):
        futures = [rpc_async(w, self.dataset, self.args, self.kwargs, rref=True) for w in self.workers]
