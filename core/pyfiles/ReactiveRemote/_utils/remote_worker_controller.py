...
class workerManager():
    ...
    def _remote_try_put_index(self):
        ...
        if self._remote_task_queue.qsize() < self._remote_prefetch_factor:
            try:
                ...
                self._send_idx += 1
                self._update_worker_progress_info(None)
            except StopIteration:
                self._intentional_stop_event.set()
                # return
                return WM_STOP_ITERATION_SIGNAL
    ...
    def _GPU_try_put_index(self):
        ...
        num_microbatches = 1  # 4 // self._microbatch_size
        microbatches_to_send_idx = 0
        ...
        if microbatches_to_send:
            ...
            self._gpu_task_queue.put((CO.start_op(), (microbatches_to_send_idx, microbatches_to_send)))   
        return WM_CONTINUE_ITERATION_SIGNAL    
...
def _control_loop(rank, index_queues, task_queue, index_sampler, worker_control_events, _intentional_stop_event, prefetch_factor, 
                      ...
                      worker_status, dataset, world_size, gpu_task_queue, single_sample):
        ...
        while not worker_controller_stop_event.is_set():
            ...
            while iter_start_event.is_set():
                ...
                if wm._THREHOLD_FLAG:
                    ...
                    if unsent_tasks == 0 and wm._gpu_task_queue.qsize() == 0:
