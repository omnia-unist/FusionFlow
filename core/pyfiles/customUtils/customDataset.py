...
#     def __init__(self, saveto, maxlen=50):
...
#                 id_to_text[id] = text
#         self.data = self.get_data(wavs, id_to_text, maxlen)
#         self.vectorizer = VectorizeChar(maxlen)
#         print("vocab size", len(self.vectorizer.get_vocabulary()))
        
#     def __getitem__(self, idx):
#         audio_path = self.data[idx]["audio"]
#         text = self.data[idx]["text"]
#         audio = self.path_to_audio(audio_path)
#         text_encoded = self.vectorizer.encode(text)
#         audio_len = audio.shape[1]
#         text_len = len(text_encoded)
#         # print(audio.shape, text_encoded, "HAHA", flush=True)
#         # return {"source": audio, "target": text_encoded, "source_len": audio_len, "target_len": text_len}
#         return audio, text_encoded
        
#     def __len__(self):
#         return len(self.data)
    
#     def get_data(self, wavs, id_to_text, maxlen):
...
#         audio = audio.squeeze()
#         stfts = torch.stft(audio, n_fft=256, hop_length=80, win_length=200, window=torch.hann_window(200))
#         x = torch.pow(torch.abs(stfts), 0.5)
#         # normalization
#         means = torch.mean(x, dim=1, keepdim=True)
#         stddevs = torch.std(x, dim=1, keepdim=True)
#         x = (x - means) / stddevs
#         audio_len = x.shape[1]
#         # padding to 10 seconds
#         pad_len = 2754
#         pad = nn.ConstantPad2d((0, 0, 0, pad_len - audio_len), 0)
#         # print(x.shape, "SHAPE BEFORE PADDING")
#         x = pad(x)[:pad_len, :]
#         return x


class SegregatedAugImageFolder(ImageFolder):
    def __init__(
            self,
            *args,
            **kwargs
    ...
    def __getitem__(self, index):
        ...
        # if self.target_transform is not None:
        #     target = self.target_transform(target)
        # return np.array(self.transform.transforms[0](sample)), target
        return sample, target
    
    def _get_transformed_item(self, index):
        path, target = self.samples[index]
        sample = self.loader(path)
        if self.transform is not None:
            sample = self.transform(sample)
