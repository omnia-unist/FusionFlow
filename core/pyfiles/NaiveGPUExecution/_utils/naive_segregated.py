import operation_offloading as CO
...
def main(dataset):
    ...
    eii_pipes = ExternalInputGpuIterator(dataset, microbatch_size)
    
    @experimental.pipeline_def(enable_conditionals=False)
    def decode_augment_then_FlipAndNormalize_pipeline(
                                     augmentations=[],
                                     std=(0.229 * 255, 0.224 * 255, 0.225 * 255),
                                     mean=(0.485 * 255, 0.456 * 255, 0.406 * 255),
                                     crop=224):
        jpegs, labels = dali.fn.external_source(
                        source=eii_pipes, num_outputs=2, device="cpu")
        images = fn.decoders.image(jpegs, device="mixed")
        rng = fn.random.coin_flip(probability=0.5)
        images = fn.decoders.image(jpegs, device="mixed")
        ...
        for aug in augmentations:
            images = aug(images)
            # for crop and etc, it must be in UINT8
            images = dali.fn.cast(images, dtype=types.DALIDataType.UINT8)
    ...
    if "RandAugment_Manual" in OP_STAGE_ORDER:
        aug_factor = CO.M
        for op, minval, maxval in CO.AUG_NAME_LIST:
            augmentation_functions[op] = augmentation_func(op, minval, maxval, aug_factor)
    elif "AutoAugment_Manual" in OP_STAGE_ORDER:
        for i, policy in enumerate(CO.POLICY_LIST):
            policy_name = "Policy " + str(i)
            augmentation_functions[policy_name] = []
            for aug_name, prob, aug_factor in policy:
                # Get Min-Max Value
                for op, minval, maxval in CO.AUG_NAME_LIST:
                    if op == aug_name:
                        augmentation_functions[policy_name].append((prob, augmentation_func(op, minval, maxval, aug_factor)))
                        break
    ...
    while 1:
        ...
        while True:
            ...
            if op_name in augmentation_functions:
                if type(augmentation_functions[op_name]) is not list:
                    augmenations.append(augmentation_functions[op_name])
                else:
                    for prob, aug in augmentation_functions[op_name]:
                        if random.random() > prob:
                            continue
                        augmenations.append(aug)
            elif op_name != "FlipAndNormalize" and op_name != "RandomCrop":
                print("This operation is not implemented", op_name, flush=True)
                raise NotImplementedError()
            op_type = CO.next_op(OP_STAGE_ORDER, op_type)
            if CO.is_finish(op_type):
                break
        one_time_gpu_pipeline = decode_augment_then_FlipAndNormalize_pipeline(
                                                             augmentations=augmenations,
                                                             num_threads=workers, batch_size=max_batch_size, 
                                                             device_id=_rank, prefetch_queue_depth=1)
        one_time_gpu_pipeline.build()
        _input, _target = one_time_gpu_pipeline.run()
        ...
        # Destroy Pipes
        del one_time_gpu_pipeline
        one_time_gpu_pipeline = None

if __name__ == '__main__':
  traindir = "/data/openimage/train"
