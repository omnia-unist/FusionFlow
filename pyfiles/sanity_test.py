...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader_ori = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=False, persistent_workers=True, drop_last=True)

    train_loader = PinData.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=False, rank = cur_rank,
        num_workers=args.workers, pin_memory=False, persistent_workers=True, drop_last=True, worker_batch_size=args.threads)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        out1 = train(train_loader, model, criterion, optimizer, epoch, args)

    # origin
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        out2 = train(train_loader_ori, model, criterion, optimizer, epoch, args)

    for i in range(len(out2)):
        try:
            if torch.equal(out1[i], out2[i]):
                print("PASS")
            else:
                print("FAIL: ValueError: Please fix this issue.")
        except:
            print("FAIL: IndexError: Please fix this issue.")
