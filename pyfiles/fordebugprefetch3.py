...
import random
import numpy as np
import MicroBatchLoader as PinData # custom

# import PinData as PinData # custom
from globalManager import native as ngc
import logging  
if __debug__:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # if args.gpu == 0:
    #     # init server
    #     ngc.globalManager(args.rank, dist.get_world_size(), args.batch_size)
    ...
    if args.tracefetch:
        train_dataset = customUtils.FullTraceCopyedDistMemDataset(
            cur_rank,
            traindir,
    ...
    if args.distributed:
        ...
        if __debug__ and args.fulltrace:
            train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, prefetch_factor=3)
        else:
            train_loader = PinData.DataLoader(
                train_dataset, rank = cur_rank,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, worker_batch_size=args.threads, prefetch_factor=3)
    else:
        if args.ddpmanual:
            train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, rank=args.gpu)
            batch_sampler = customUtils.OverwriteBatchSampler(train_sampler, args.batch_size, drop_last=True, num_workers=args.workers, prefetch_factor=3)
        else:
            train_sampler = None
        ...
        if args.ddpmanual:
            ...
            train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, worker_batch_size=args.threads, prefetch_factor=3)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # images= torch.cat((images,sample), dim=0)
        # print(np.shape(images))
        ...
        if __debug__:
            log.debug(f"Finish iteration{i}")
