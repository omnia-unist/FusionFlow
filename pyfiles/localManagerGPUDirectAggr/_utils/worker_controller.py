...
class workerManager():
    def __init__(self,_workers_control_events,_max_workers,_worker_queue_idx_cycle,
                    ...
                    microbatch_size, worker_status, dataset, world_size, gpu_task_queue, gpu_dali_queue) -> None:
        self._rank = _rank
        ...
        self._gpu_dali_queue = gpu_dali_queue
        ...
        self._minibatch_size = int(self._microbatch_to_minibatch * self._microbatch_size)
        ...
        _worker_progress = self._load_np_arr("worker_progress_info", worker_id=self._max_workers, frame=np.array([-1, -1], dtype=gc.INFO_DATA_TYPE), dtype=gc.INFO_DATA_TYPE)
    ...
    def _reset(self):
        ...
        # GPU only progress start from 0 
    ...
    def _GPU_try_put_index(self):
        ...
        for _ in range(1):
            if self._gpu_task_queue.qsize() < self._gpu_prefetch_factor and self._gc_helper.Get_progress_iter() >= self._task_batch:
                try:
                    index = self._next_index()
                    self._gpu_task_queue.put(self._send_idx)
                    self._gpu_dali_queue.put(index)
                    # Put dummy to push current index...
                    self._gpu_dali_queue.put([0])
    ...
    def _check_queue_put_index(self):
        for worker_queue_idx in self._worker_info:  # find the next active worker, if any
            ...
            if worker_queue_idx in self._mark_to_return:
                ...
                self._update_worker_progress_info(worker_id)
                continue
            elif not self._workers_control_events[worker_id].is_set():
                continue
            if self._index_queues[worker_id].qsize() < self._prefetch_factor:
                if len(self._stray_task_buffer):
                    ...
                    if task[0] < self._next_task_idx:
                        self._index_waiting_queue[worker_id].append(task[0])
                        self._queue_tasks_counter += 1
                    else:
                        self._next_index_waiting_queue[worker_id].append(task[0])
                    ...
                    self._task_info[task[0]] = (worker_id,)
                else:
                    try:
                        index = self._next_index()
                    except StopIteration:
...
def _control_loop(rank, index_queues, task_queue, index_sampler, worker_control_events, _intentional_stop_event, prefetch_factor, 
                      ...
                      worker_status, dataset, world_size, gpu_task_queue, gpu_dali_queue):
    ...
    wm = workerManager(_rank=rank, _workers_control_events=worker_control_events, _max_workers=max_workers, _cur_cpus=cur_cpus,
                       ...
                       worker_status=worker_status, dataset=dataset, world_size=world_size, gpu_task_queue=gpu_task_queue, gpu_dali_queue=gpu_dali_queue)
    try:
