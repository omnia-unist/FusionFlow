...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        # self._aggr_template = [torch.empty(self._batch_size, self._aggr_shape_hint[0], self._aggr_shape_hint[1], self._aggr_shape_hint[2], device = self._rank),
        #                        torch.empty(self._batch_size, dtype=torch.long, device = self._rank)]
        ...
        self.gpu_done_events = multiprocessing_context.Event()
        ...
        # self._naive_microbatch = 1
        self.gpu_worker_process = multiprocessing_context.Process(
                target=_utils.dali._gpu_worker_loop,
                args=(self._gpu_index_queue, self._gpu_dali_queue, self._worker_result_queue, self._dataset, self._microbatch_size,
                      self._rank, self._world_size, 1, self.gpu_done_events, self._gpu_finish_event, self._max_workers, self._aug_type, self._aggr_shape_hint))
        ...
        #         type="worker_progress_info", worker_id=i, shm_arrs=self._worker_progress_info, np_arr=np.array([-1, -1], dtype=gc.INFO_DATA_TYPE), dtype=gc.INFO_DATA_TYPE)
            ...
            #           self.cur_cpus, loader.dataloader_processes, self._rank))
        ...
        self.worker_control_process = multiprocessing_context.Process(
                ...
                args=(self._rank, self._index_queues, self._task_queue, self._index_sampler, self._workers_control_events, self._intentional_stop_event, self._prefetch_factor, 
                      ...
                      self._microbatch_size, self._workers_status, self._dataset, self._world_size, self._gpu_index_queue, self._gpu_dali_queue))
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        ...
        complete_data = [torch.empty(self._aggr_shape_hint, device = self._rank),
                        torch.empty(self._batch_size, dtype=torch.long, device = self._rank)]
        self._try_check_task_queue()
    ...
    def _shutdown_workers(self):
        ...
        if not self._shutdown:
            ...
            try:
                ...
                if hasattr(self, "gpu_worker_process"):
                    self._gpu_index_queue.put(None)
                    self.gpu_done_events.set()
                    self.gpu_worker_process.join()
                    if self.gpu_worker_process.is_alive():
                        self.gpu_worker_process.terminate()
                    self._gpu_dali_queue.cancel_join_thread()
                    self._gpu_dali_queue.close()
                    self._gpu_index_queue.cancel_join_thread()
                    self._gpu_index_queue.close()
                # Exit workers now.
