...
import Augmix
import numpy as np
import SchedulerAggrSplitLoader as PinData
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for iter_num, (images, target) in enumerate(train_loader):
        ...
        if __debug__:
            log.debug(f"GPU {args.gpu} Iteration{iter_num} done")
...
def validate(val_loader, model, criterion, args):
    ...
    model.eval()
    halfrun = len(val_loader)//2
    with torch.no_grad():
        end = time.perf_counter()
        for iter_num, (images, target) in enumerate(val_loader):
            dtime = datetime.datetime.now()
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True)
            if torch.cuda.is_available():
            ...
            if iter_num % args.print_freq == 0:
                progress.display(iter_num, dtime)
