        ...
        while True:
            ...
            if len(self._task_info[self._rcvd_idx]) == 2:
                ...
                if __debug__:
                    ...
                    log.debug(f"GPU: {self._rank} Processing data: {end-start} idx {self._rcvd_idx} ")
            ...
            if idx != self._rcvd_idx:
                ...
                if __debug__:
                    ...
                    log.debug(f"GPU: {self._rank} Buffer data: {end-start} idx {idx}")
            else:
                self._process_data(complete_data, data)
                ...
                if __debug__:   
                    ...
                    log.debug(f"GPU: {self._rank} Processing data: {end-start} idx {idx}")
            
    def _check_queue_put_index(self):
        for worker_queue_idx in range(self._num_workers):  # find the next active worker, if any
            ...
            if self._index_queues[worker_queue_idx].qsize() < self._prefetch_factor:
                ...
                if __debug__:
                    log.debug(f"PUT ({self._send_idx}, {index}) to worker_queue_idx{worker_queue_idx} _index_queues:\n{self._index_queues}")
