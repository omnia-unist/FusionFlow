...
import datetime
import RandAugment
import FastAutoAugment # custom
import customUtils # custom
import Augmix # Augmix
import random
import numpy as np
import localManagerCPU as PinData # custom
# import PinData as PinData # custom
import logging  
...
parser.add_argument('--fulltrace', action='store_true',
                    help='Testing with small dataset')
parser.add_argument('--ddpmanual', action='store_true',
                    help='DDP manaul')
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        mp.spawn(main_worker, nprocs=4, args=(ngpus_per_node, args))
    else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.multiprocessing_distributed:
            ...
            gc.globalManager_init(workers, batch_size, args.threads, controllerClass=gc.IntraInterFIFOglobalManager, rank=args.rank)
    ...
    if args.augs == "default":
    ...
    elif args.augs == "noresizecrop":
        augmentations=[
                transforms.RandomHorizontalFlip(p=1.0),
                transforms.ToTensor(),
                normalize
            ]
    elif args.augs == "noresize":
        augmentations=[
        ...
        augmentations = [transforms.RandomResizedCrop(size),
                         transforms.RandomHorizontalFlip()]
    ...
    print("maxworkers:",(NODE_MAX+1)*len(CPU_IDS))
    ...
    if args.augs == "augmix":
        ...
        train_dataset = Augmix.augmixDataset.AugMixDataset(train_dataset, preprocess, no_jsd=True)
    
    
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
        # train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, rank=cur_rank)
        # batch_sampler = customUtils.OverwriteBatchSampler(train_sampler, args.batch_size, drop_last=True, num_workers=args.workers)
        if __debug__ and args.fulltrace:
            batch_sampler = None
            train_loader = PinData.DataLoader(
                train_dataset, rank = cur_rank,
                num_workers=args.workers, pin_memory=False, shuffle=(train_sampler is None), sampler=train_sampler, batch_size=args.batch_size, persistent_workers=True, batch_sampler=batch_sampler, worker_batch_size=args.threads, drop_last=True, max_workers=CPU_NUM, aggr_shape_hint=template_shape)
        else:
            batch_sampler = None
            train_loader = PinData.DataLoader(
                train_dataset, rank = cur_rank,
        ...
        if args.ddpmanual:
            batch_sampler = None
            # train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, rank=args.gpu)
            # batch_sampler = customUtils.OverwriteBatchSampler(train_sampler, args.batch_size, drop_last=True, num_workers=args.workers)
            train_loader = PinData.DataLoader(
                train_dataset, rank = cur_rank,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, max_workers=CPU_NUM, aggr_shape_hint=template_shape)
        else:
            train_sampler = None
            # train_sampler = customUtils.PrepSampler(train_dataset)
            batch_sampler = None
            train_loader = PinData.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, worker_batch_size=args.threads, max_workers=CPU_NUM, aggr_shape_hint=template_shape)
        
    val_loader = torch.utils.data.DataLoader(
                customUtils.SharedImageFolder(valdir, transforms.Compose([
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if __debug__:
            log.debug(f"GPU: {args.rank} Finish iteration{i} IterTime:{itertime}")
