...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        self._aggr_template = [torch.empty((self._batch_size, 3, 224, 224), dtype=torch.float),
                               torch.empty(self._batch_size, dtype=torch.float)]
        ...
        self._controller_queue = multiprocessing_context.Queue()
        ...
        self._minibatch_done_event = multiprocessing_context.Event()
        self._minibatch_done_event.clear()
        ...
        self._iteration_end_status = multiprocessing_context.Value(c_bool, False)
        self._index_queues = []
        self._workers = []
        self._worker_progress_info = []
        self._internal_shm_metas = []
        self._gpu_index_queue = multiprocessing_context.Queue()
        ...
        self.worker_control_process = multiprocessing_context.Process(
                ...
                args=(self._rank, self._index_queues, self._index_sampler, self._workers_control_events, self._prefetch_factor, 
                      ...
                      self._microbatch_to_minibatch, self._persistent_workers, self._num_workers, loader.dataloader_processes, self._iteration_end_status,
                      self._microbatch_size, self._workers_status, self._aggr_template, self._data_queue, self._minibatch_done_event,
                      self._worker_batch_target_counter, self._controller_queue))
    ...
    def _save_np_arr_to_shm(self, type, worker_id, shm_arrs, np_arr, dtype):
        ...
        try:
            ...
            nbytes = sys.getsizeof(np_arr.storage)
    ...
    def _reset(self, loader, first_iter=False):
        ...
        self._complete_batch_counter = 0
        self._iteration_end_status.value = False
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        if __debug__:
            log.debug(f"GPU: {self._rank} Enter Minibatch Iteration")
        while self.worker_control_process.is_alive():
            minibatch = self._controller_queue.get()
            # if __debug__:
            #     start = time.perf_counter()
            if minibatch == None:
                if __debug__:
            ...
            # if __debug__:
            #     end = time.perf_counter()
            #     log.debug(f"GPU: {self._rank} Return MiniBatch at_time {end-start}")
            return minibatch
        raise RuntimeError("Cannot find worker controller process")
        ...
        if not self._shutdown:
            ...
            try:
                ...
                if hasattr(self, 'worker_control_process'):
                    ...
                    self._controller_queue.cancel_join_thread()
                    self._controller_queue.close()
