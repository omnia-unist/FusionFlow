...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.tracefetch:
        ...
        shm_mngr = managers.SharedMemoryManager()
        shm_mngr.start()
        train_dataset = customUtils.SharedMemoryDistMemDataset(
            traindir,
            augmentations_composed,
            shm_mngr = managers.SharedMemoryManager()
        )
    ...
    if args.distributed:
        train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, num_replicas=4, rank=cur_rank)
        ...
        if args.ddpmanual:
            train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, num_replicas=4, rank=args.gpu)
            batch_sampler = customUtils.OverwriteBatchSampler(train_sampler, args.batch_size, drop_last=True, num_workers=args.workers)
        else:
            train_sampler = None
