...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        train_sampler = customUtils.PrepDistributedSampler(train_dataset)
    ...
    start_cpu_id = CPU_IDS_PER_GPU * args.gpu + args.workers
        ...
        if args.threads == 0:
            ...
            num_workers=args.workers, pin_memory=False, persistent_workers=False, batch_sampler=batch_sampler)
        else:
            pass
            # train_loader = PinData.DataLoader(
            # train_dataset, 
            # num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, max_threads = args.threads)

        if __debug__ and args.fulltrace:
            train_loader = FullTraceData.DataLoader(
            train_dataset, rank = cur_rank,
            num_workers=args.workers, pin_memory=False, persistent_workers=False, batch_sampler=batch_sampler)
        ...
        if args.threads == 0:
            ...
            num_workers=args.workers, pin_memory=False, persistent_workers=False, sampler=train_sampler, drop_last=True)
        else:
            raise NotImplementedError("In Progress multithreading")
            # To Do: need to update with multithreading dataloader
            #
            # train_loader = PinData.DataLoader(
            # train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
            # num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, max_threads = args.threads, drop_last=True)
