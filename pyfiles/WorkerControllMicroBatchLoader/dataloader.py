...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
                 ...
                 *, prefetch_factor: int = 2,
                 ...
                 rank: Optional[int] = 0,
                 worker_batch_size: Optional[int] = 8,
                 aggr_shape_hint: Optional[tuple] = (3, 224, 224),
                 max_workers: Optional[int] = None,
                 max_gpu: Optional[int] = 8,
                 debug_increase: Optional[int] = 0,
                 debug_decrease: Optional[int] = 0 ):
        ...
        # prefetch_min = batch_size/worker_batch_size/num_workers
        # if prefetch_factor < prefetch_min:
        #     self.prefetch_factor = int(prefetch_min)
        # else:
        ...
        self.gpu_per_node = max_gpu // (self.node_max+1)
        ...
        if max_workers is None:
            self.max_workers = len(self.cur_cpus)
        else:
        ...
        if __debug__:
            log.debug(f"GPU: {self.rank}, cpu_id_start: {self.cpu_id_start}, cpu_per_gpu: {self.cpu_per_gpu}, max_workers: {self.max_workers}")
        ...
        if batch_size is not None and batch_sampler is None:
            ...
            batch_sampler = FinegrainedBatchSampler(batch_size = worker_batch_size, sampler = sampler, actual_batch_size = batch_size, drop_last = drop_last)
        ...
        self.debug_increase = debug_increase
        self.debug_decrease = debug_decrease
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader: DataLoader) -> None:
        ...
        self._debug_increase = loader.debug_increase
        self._debug_decrease = loader.debug_decrease
        self._num_yielded = 0
        self._rank = loader.rank
        self._batch_size = loader.batch_size
        self._worker_batch_size = loader.worker_batch_size
    ...
    def _reset(self, loader, first_iter=False):
        ...
        if __debug__:
            ...
            log.debug(f"Create batch sample:\n{end - start}")
        self._num_yielded = 0
        self._IterableDataset_len_called = loader._IterableDataset_len_called
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def _worker_decrease(self, cpus, timeout=_utils.MP_CONTROL_CHECK_INTERVAL):
        prev_workers = self._num_workers
        next_workers = self._num_workers-len(cpus)
        self._num_workers = next_workers if next_workers > 1 else 1
        if __debug__:
            log.debug(f"GPU: {self._rank} Decrease worker = {self._num_workers}, cpus: {cpus}")
        
        reclaimed_tasks = []
        for cpu_id in cpus:
            # Make it to available worker
            worker_id = cpu_id - self.cur_cpus[0]
            # if __debug__:
            #     log.debug(f"GPU: {self._rank} Off Worker id {worker_id}")
            ...
            while self._index_queues[worker_id].qsize() > 0:
                try:
                    ...
                    break
                reclaimed_tasks.append(data)
        if __debug__:
            log.debug(f"GPU: {self._rank} Reclaimed tasks: {reclaimed_tasks}")

        if reclaimed_tasks:
            self._try_put_target_tasks(reclaimed_tasks)

    def _worker_increase(self, cpus):
        next_workers = self._num_workers+len(cpus)
        self._num_workers = next_workers if next_workers < self._max_workers else self._max_workers
        if __debug__:
            log.debug(f"GPU: {self._rank} Increase worker = {self._num_workers}, cpus: {cpus}")
            
        # NOTE: Add index queue directly when increase workers only
    ...
    def _worker_control(self):
        if self._shutdown:
            if __debug__:
                log.debug(f"GPU: {self._rank} Workers already shutdown, so cannot control")
            return
        # FIXME: Fix worker control mechnaism
        head_cpu_id = self._tail_cpu_id+self._num_workers-1
        # if self._rank:
        less_cpus = head_cpu_id-self._debug_increase
        if __debug__:
            log.debug(f"GPU: {self._rank} tail_cpu_id: {self._tail_cpu_id}, head_cpu_id: {head_cpu_id}, less_cpus: {less_cpus}")
        self._worker_decrease(self.cur_cpus[less_cpus:head_cpu_id])
        
        head_cpu_id = self._tail_cpu_id+self._num_workers-1
        # if self._toggle:
        more_cpus = head_cpu_id+self._debug_decrease
        if __debug__:
            log.debug(f"GPU: {self._rank} tail_cpu_id: {self._tail_cpu_id}, head_cpu_id: {head_cpu_id}, more_cpus: {more_cpus}")
        self._worker_increase(self.cur_cpus[head_cpu_id:more_cpus])
        # else:
        # else:
        #     if self._toggle:
        #         more_cpus = self._tail_cpu_id-self._debug_increase
        #         if __debug__:
        #             log.debug(f"GPU: {self._rank} tail_cpu_id: {self._tail_cpu_id}, head_cpu_id: {head_cpu_id}, more_cpus: {more_cpus}")
        #         self._worker_increase(self.cur_cpus[more_cpus:self._tail_cpu_id])
        #         self._tail_cpu_id = more_cpus
        #     else:
        #         less_cpus = self._tail_cpu_id+self._debug_increase
                
        #         if __debug__:
        #             log.debug(f"GPU: {self._rank} tail_cpu_id: {self._tail_cpu_id}, head_cpu_id: {head_cpu_id}, less_cpus: {less_cpus}")
        #         self._worker_decrease(self.cur_cpus[self._tail_cpu_id:less_cpus])
        #         self._tail_cpu_id = less_cpus
    ...
    def _reset(self, loader, first_iter=False):
        ...
        if __debug__:
            log.debug(f"GPU: {self._rank} tail_cpu_id: {self._tail_cpu_id} num_workers: {self._num_workers}")
        
        # We resume the prefetching in case it was enabled
        if not first_iter:
            for idx in range(self._max_workers):
                if not self._workers_control_events[idx].is_set():
                    continue
                self._index_queues[idx].put(_utils.worker._ResumeIteration())
                if __debug__:
                    log.debug(f"PUT {_utils.worker._ResumeIteration()} to worker{idx}")
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        ...
        # FIXME: Hard-code for image data only
        ...
        while True:
            ...
            while self._rcvd_idx < self._send_idx:
                ...
                worker_id = info[0]
                if len(info) == 2 or self._workers_status[worker_id].value:  # has data or is still active
                    break
                # FIXME for support iterable dataset file
            ...
            if len(self._task_info[self._rcvd_idx]) == 2:
                ...
                if __debug__:
                    ...
                    log.debug(f"GPU: {self._rank} Process Buffer data: {end-start} idx {self._rcvd_idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
                
                if self._worker_batch_counter >= self._worker_batch_target_counter or self._shutdown:
                    if self._debug_decrease != 0 and self._debug_increase != 0:
                        if __debug__:
                            start_time=time.perf_counter()
                        self._worker_control()
                        if __debug__:
                            end_time=time.perf_counter()
                            log.debug(f"Control time: {end_time - start_time}")
                    if __debug__:
                        end=time.perf_counter()
                        log.debug(f"GPU: {self._rank} Complete data: {end-start} idx {self._rcvd_idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
                    return complete_data
                
                # if __debug__:
                #     end=time.perf_counter()
                #     log.debug(f"GPU: {self._rank} Processing data: {end-start} idx {self._rcvd_idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
            ...
            if idx != self._rcvd_idx:
                ...
                if __debug__:
                    ...
                    log.debug(f"GPU: {self._rank} Buffer data: {end-start} idx {idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
            else:
                self._process_data(complete_data, data)
                ...
                if self._worker_batch_counter >= self._worker_batch_target_counter or self._shutdown:
                    ...
                    if __debug__:
                        ...
                        log.debug(f"GPU: {self._rank} Complete data: {end-start} idx {idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
                    return complete_data
                
                if __debug__:
                    end=time.perf_counter()
                    log.debug(f"GPU: {self._rank} Processing data: {end-start} idx {idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
                
            
    def _check_queue_put_index(self):
        for worker_queue_idx in range(self._max_workers):  # find the next active worker, if any
    ...
    def _try_put_index(self):
        ...
        self._send_idx += 1
        # if __debug__:
        #     log.debug(f"tasks_outstanding up to {self._tasks_outstanding}")
    ...
    def _process_data(self, complete_data, data):
        ...
        if isinstance(data, ExceptionWrapper):
            data.reraise()
        next_worker_batch = self._worker_batch_counter + self._worker_batch_size
        ...
        if __debug__:
            log.debug(f"GPU: {self._rank} Process worker_batch_counter: {self._worker_batch_counter}, next_worker_batch: {next_worker_batch}")
