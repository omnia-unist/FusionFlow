...
def _gpu_worker_loop(in_queue, dali_queue, out_queue, dataset, microbatch_size, _rank, world_size, workers, done_event, gpu_finish_event, gpu_worker_id, augmentation_type, aggr_shape_hint):
    ...
    if __debug__:
        log.debug(f"GPU: {_rank} GPU worker size template: {aggr_shape_hint} {aggr_shape_hint[3]}")
    dali_processor = DALI_GPU_Processor(dataset, microbatch_size, _rank, world_size=world_size, workers=workers, in_queue=dali_queue, size = aggr_shape_hint[3], augment_type= augmentation_type)
    if __debug__:
    ...
    while not done_event.is_set():
        ...
        while not done_event.is_set():
            ...
            if __debug__:
                log.debug(f"GPU:{_rank} GPU workers check dummy {len(_input)}")
        ...
        torch_input=torch.empty(microbatch_size,aggr_shape_hint[1],aggr_shape_hint[2],aggr_shape_hint[3],device=_rank, dtype=torch.float32)
        torch_target=torch.empty(microbatch_size,device=_rank, dtype=torch.uint8)
        feed_ndarray(input_tensor, torch_input, torch.cuda.current_stream(device=_rank), torch_input.data_ptr())
        feed_ndarray(target_tensor, torch_target, torch.cuda.current_stream(device=_rank), torch_target.data_ptr())
        if __debug__:
            log.debug(f"GPU:{_rank} see target {target_tensor} and copyed {torch_target}")
        del _input, _target, input_tensor, target_tensor
        cpu_data = [torch_input.cpu(), torch_target.cpu()]
        while not done_event.is_set():
            try:
                out_queue.put((idx,cpu_data))
                break
            except queue.Full:
                continue
