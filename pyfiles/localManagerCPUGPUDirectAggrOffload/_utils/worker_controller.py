...
class workerManager():
    def __init__(self,_workers_control_events,_max_workers,_worker_queue_idx_cycle,
                    ...
                    microbatch_size, worker_status, world_size, gpu_task_queue, gpu_dali_queue,
                    single_sample) -> None:
        self._rank = _rank
            ...
            if self._gpu_task_queue.qsize() < self._gpu_prefetch_factor and self._gc_helper.Get_progress_iter() >= self._task_batch:
                try:
                    ...
                    if self._single_sample:
                        ...
                        for single_sample in index:
                            self._gpu_dali_queue.put([single_sample])
...
def _control_loop(rank, index_queues, task_queue, index_sampler, worker_control_events, _intentional_stop_event, prefetch_factor, 
                      ...
                      worker_status, world_size, gpu_task_queue, gpu_dali_queue, single_sample):
    ...
    wm = workerManager(_rank=rank, _workers_control_events=worker_control_events, _max_workers=max_workers, _cur_cpus=cur_cpus,
                       ...
                       worker_status=worker_status, world_size=world_size, gpu_task_queue=gpu_task_queue, gpu_dali_queue=gpu_dali_queue,
