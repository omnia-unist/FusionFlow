...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
        ...
        self.gc_helper = gc.globalManagerHelper(rank = self.rank)
        if __debug__:
...
class _BaseDataLoaderIter(object):
    ...
    def _reset(self, loader, first_iter=False):
        ...
        if __debug__:
            ...
            log.debug(f"Create batch sample: {end - start}")
        self._num_yielded = 0
        self._IterableDataset_len_called = loader._IterableDataset_len_called
    ...
    def __next__(self) -> Any:
        ...
        if __debug__:
            log.debug(f"GPU: {self._rank} RETURN data object")
        return data
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        self._batch_size = loader.batch_size
        self._worker_batch_size = loader.worker_batch_size
        self._idx_size_to_next_batch = self._batch_size // self._worker_batch_size
    ...
    def _worker_decrease(self, cpus, timeout=_utils.MP_CONTROL_CHECK_INTERVAL):
        ...
        for task in remaining_tasks:
            ...
            if __debug__:
                log.debug(f"GPU: {self._rank} Put idx {task[0]} to stray buffer")
        ...
        if not self._intentional_stop and self._num_workers < 1:
            self._intentional_stop = True
            if hasattr(self, '_pin_memory_thread'):
                self._pin_memory_intentional_stop_solved_event.clear()
        
        if __debug__:
            log.debug(f"GPU: {self._rank} Decrease worker = {self._num_workers}, cpus: {cpus}, valid: {valid_cpu_ids}, intentional_stop: {self._intentional_stop}")
        
    def _check_move_to_next_batch(self):
        if self._send_idx % self._idx_size_to_next_batch == 0:
        ...
        for cpu_id in cpus:  # find the next active worker, if any
            ...
            if self._index_queues[worker_id].qsize() < self._prefetch_factor:
                if len(self._stray_task_buffer):
                    ...
                    self._send_idx += 1
                    self._check_move_to_next_batch()
            
        # Update num workers
        if self._num_workers > self._max_workers:
            self._num_workers = self._max_workers

        # Check stopped intentionally
        if self._intentional_stop and self._num_workers > 0:
            self._intentional_stop = False
            if hasattr(self, '_pin_memory_thread'):
                self._pin_memory_intentional_stop_solved_event.set()  
        
        if __debug__:
            log.debug(f"GPU: {self._rank} Increase worker = {self._num_workers}, cpus: {cpus}, valid: {valid_cpu_ids}, intentional_stop: {self._intentional_stop}")
    ...
    def _reset(self, loader, first_iter=False):
        ...
        self._worker_batch_counter = 0
        self._complete_batch_counter = 0
        self._gc_helper.Send(self._complete_batch_counter, self._worker_batch_counter)
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        ...
        if __debug__:
            log.debug(f"GPU: {self._rank} Enter the next_data rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
        ...
        while True:
            ...
            if self._intentional_stop:
                # if __debug__:
                #     log.debug(f"GPU: {self._rank} intentional stop")
            ...
            while self._rcvd_idx < self._send_idx:
                ...
                worker_id = info[0]
                if __debug__:
                    log.debug(f"GPU: {self._rank} test idx {self._rcvd_idx}, info len: {len(info)} worker_id: {worker_id}, worker_status: {self._workers_status[worker_id].value}")
                if len(info) == 2 or worker_id == -1 or self._workers_status[worker_id].value:  # has data or on stray or is still active
                    break
                del self._task_info[self._rcvd_idx]
                if __debug__:
                    log.debug(f"GPU: {self._rank} delete idx {self._rcvd_idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}, worker_id: {worker_id}")
                self._rcvd_idx += 1
            else:
                # Handle for remaining data
                # if self._worker_batch_counter != 0:
                #     if __debug__:
                #         log.debug(f"GPU: {self._rank} remaining complete data idx {self._rcvd_idx}, rcvd_idx: {self._rcvd_idx} send_idx: {self._send_idx}")
                #     # self._worker_reset()
                #     
                #     return complete_data[:self._worker_batch_counter]
                if __debug__:
                    log.debug(f"GPU: {self._rank} Stop Iteration")
    ...
    def _process_data(self, complete_data, data):
        ...
        self._gc_helper.Send(current_batch=self._complete_batch_counter, 
                             micro_batch_num=self._worker_batch_counter)
    ...
    def _shutdown_workers(self):
        if __debug__:
            log.debug(f"GPU: {self._rank} Shutdown workers rcvd_idx: {self._rcvd_idx}, send_idx: {self._send_idx}")
