    ...
    if args.distributed:
        ...
        if __debug__ and args.fulltrace:
            train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, prefetch_factor=1)
        else:
            train_loader = PinData.DataLoader(
                train_dataset, rank = cur_rank,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, batch_sampler=batch_sampler, worker_batch_size=args.threads, prefetch_factor=1)
    else:
        if args.ddpmanual:
            train_sampler = customUtils.PrepDistributedSampler(dataset=train_dataset, rank=args.gpu)
            batch_sampler = customUtils.OverwriteBatchSampler(train_sampler, args.batch_size, drop_last=True, num_workers=args.workers, prefetch_factor=1)
        else:
            train_sampler = None
        ...
        if args.ddpmanual:
            ...
            train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, worker_batch_size=args.threads, prefetch_factor=1)
