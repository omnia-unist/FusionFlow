...
import daliRandAgument as dalira
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        traindir = os.path.join(args.data, 'train')
        for_classes = datasets.ImageFolder(
                    traindir,
                )
        model =  ViT(
            image_size = 256,
            patch_size = 32,
            num_classes = len(for_classes.classes),
            dim = 1024,
            depth = 6,
            heads = 16,
            mlp_dim = 2048,
    ...
    if __debug__:
        log.debug(f"arch: {args.arch}, size {size}")
    ...
    if args.seed is None:
        if __debug__:
            log.info(f"No seed")
        train_loader = dalira.dali.DaliMicroBatchLoader(dataset=train_dataset, device_id=0 if args.gpu == None else args.gpu, template_shape=template_shape, path=traindir,
            microbatch_size=args.threads, minibatch_size=args.batch_size, size=size, fp16=True, aug_type=args.augs, sampler=train_sampler, world_size=NUM_GPU
        )
        ...
        train_loader = dalira.dali.DaliMicroBatchLoader(dataset=train_dataset, device_id=0 if args.gpu == None else args.gpu, template_shape=template_shape, path=traindir,
            microbatch_size=args.threads, minibatch_size=args.batch_size, size=size, generator=g, fp16=True, aug_type=args.augs, sampler=train_sampler, world_size=NUM_GPU
        )
    
    if args.augs == "default":
        train_loader = train_loader.get_default_loader()
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        # print(f"images: {images.shape} {type(images)} target: {target.shape} {type(target)}")
