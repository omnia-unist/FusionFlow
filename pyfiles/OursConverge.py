...
import Augmix
import numpy as np
import localManagerCPUGPUDirectAggrOffload as PinData
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.augs == "default":
        ...
        augmentations.insert(0, RandAugment.augmentations.DALIRandAugment(2, 9))
        ...
        augmentations.insert(2, RandAugment.augmentations.DALIRandAugment(2, 9))
    elif args.augs == "cropautoaugment":
        print(f"Using CropAutoAugment {args.augs}")
        augmentations.insert(2, FastAutoAugment.AutoAugment(FastAutoAugment.fa_resnet50_rimagenet()))
    ...
    CPU_IDS_PER_GPU = len(CPU_IDS) // GPU_PER_NODE
    ...
    if args.seed is None:
        ...
        train_loader = PinData.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=(NODE_MAX+1)*len(CPU_IDS), aggr_shape_hint=template_shape, worker_batch_size=args.threads, aug_type=args.augs)
    else:
        if __debug__:
        ...
        train_loader = PinData.DataLoader(
                train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), rank = cur_rank, aug_type=args.augs,
                num_workers=args.workers, pin_memory=False, persistent_workers=True, sampler=train_sampler, drop_last=True, max_workers=(NODE_MAX+1)*len(CPU_IDS), aggr_shape_hint=template_shape, worker_init_fn=seed_worker, generator=g, worker_batch_size=args.threads)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best2.pth.tar')
